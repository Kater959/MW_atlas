...
.pdf-speaker{
  margin-top:16px;padding:14px 16px 14px 18px;
...
.pdf-speaker-label{
  ...
  color:#b8ff3e;margin:0 0 8px;
}
.pdf-speaker-text{
  margin:0;font-size:clamp(.92rem,1.15vw,1.05rem);line-height:1.45;
  color:#c8c4bb;font-weight:400;
}
.pdf-page-inner.has-speaker{justify-content:space-between;gap:12px}
.pdf-page-main{min-height:0;flex:1 1 auto;display:flex;flex-direction:column;justify-content:center}
.pdf-page.has-speaker .pdf-media img{max-height:min(38vh,340px)}
@media print{
...
function speakerForStep(t, s) {
  ...
  if (s.kind === "render" || s.kind === "renderAlt") return t.speakerNotes;
  if (s.kind === "cta") {
...
function buildPdfPage(item, index, total) {
  ...
  return `<section class="pdf-page ${speaker ? "has-speaker" : ""}">
    ...
    <div class="pdf-page-inner ${speaker ? "has-speaker" : ""}">
...
function openPdfExport() {
  ...
  root.innerHTML = `
    <div class="pdf-toolbar">
      <div class="mono" style="color:#b8ff3e;font-size:11px;letter-spacing:.18em;text-transform:uppercase">PDF · ${list.length} слайдов · speaker notes · без 3D</div>
      <div style="display:flex;gap:8px;flex-wrap:wrap">
