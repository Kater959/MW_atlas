...
.speaker-notes{
  margin-top:18px;padding:12px 14px 12px 16px;max-width:min(52ch,100%);
  border:1px solid rgba(184,255,62,.2);
  border-left:3px solid var(--accent);
  background:linear-gradient(135deg,rgba(184,255,62,.08),rgba(5,5,5,.72) 45%,rgba(5,5,5,.82));
  backdrop-filter:blur(12px);
...
.speaker-notes-text{
  margin:0;font-size:13px;line-height:1.45;color:var(--ink-dim);
}
.slide.is-media .speaker-notes{
  margin-top:12px;max-width:min(640px,100%);
}
.slide.has-step-bg .speaker-notes{
  background:linear-gradient(135deg,rgba(184,255,62,.1),rgba(5,5,5,.88) 45%,rgba(5,5,5,.92));
}
...
.find-us-links a{
  ...
  /* invisible hit targets over QR */
}
.find-us-links a:hover{outline:1px solid rgba(184,255,62,.45);outline-offset:2px}
...
<div class="overlay" id="findUsOverlay" role="dialog" aria-modal="true" aria-label="Как нас найти">
  ...
  <div class="overlay-body find-us-body">
...
function speakerForStep(t, s) {
  // audience sees title/body; speaker block is the monologue cue
  if (s.kind === "photo") return t.caseImplementation || t.speakerNotes || "";
  if (s.kind === "cta") {
    return "Покажите проблемную операцию. Разберём, где теряется управление — и какой оснасткой вернуть ритм.";
  }
  return t.speakerNotes || "";
}

function speakerNotesHtml(t, s) {
  const text = speakerForStep(t, s);
  if (!text) return "";
  return `<aside class="speaker-notes parallax-meta">
    <div class="speaker-notes-label">Speaker · речь</div>
    <p class="speaker-notes-text">${text}</p>
  </aside>`;
}
...
function renderThought() {
  ...
  let main = "";
  const notes = speakerNotesHtml(t, s);
  if (s.kind === "text") {
...
async function mountFindUs(stageEl) {
  ...
  linksEl.innerHTML = data.links.map((L) =>
    `<a href="${L.url}" target="_blank" rel="noopener" style="left:${L.left.toFixed(3)}%;top:${L.top.toFixed(3)}%;width:${L.width.toFixed(3)}%;height:${L.height.toFixed(3)}%" aria-label="Открыть ссылку"></a>`
  ).join("");
  stageEl.classList.remove("is-loading");
}
